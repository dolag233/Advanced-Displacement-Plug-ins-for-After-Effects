	...
	PF_LayerDef* output)
{
	out_data->my_version = PF_VERSION(MAJOR_VERSION,
		...
		BUILD_VERSION);
	/*
	//创建全局变量(对于一个生命周期大概)，虽然感觉还不如直接创建全局变量
	...
	PF_Pixel16* outP)
{
	...
	if (giP->length != 0 || giP->mode == 5) {
		...
		if (giP->mode == 5) {//Dynamics
			float vel_x = (mapP->red / (double)PF_MAX_CHAN16 - .5) * 2;//注意，现在有负数值了，也就是反向
			float vel_y = (mapP->green / (double)PF_MAX_CHAN16 - .5) * 2;
			float norm = mapP->blue / (double)PF_MAX_CHAN16;//blue通道是模长，为>=0的值
			norm *= giP->dis_vel;
	...
	PF_Pixel8* outP)
{
	...
	if (giP->length != 0 || giP->mode == 5) {
		...
		//更新的像素动力学模式
		if (giP->mode == 5) {//Dynamics
			float vel_x = (mapP->red / (double)PF_MAX_CHAN8 - .5) * 2;//注意，现在有负数值了，也就是反向
			float vel_y = (mapP->green / (double)PF_MAX_CHAN8 - .5) * 2;
			float norm = mapP->blue / (double)PF_MAX_CHAN8;//blue通道是模长，为>=0的值
			norm *= giP->dis_vel;
			int MAX_ITER_TIMES = giP->max_iter;
			const float distance_square = giP->max_distance;
			float current_distance_square = .0;
	...
	PF_LayerDef* output)
{
	...
	//in_data->output_origin_x;//输入buffer的原点在输出buffer中的x位置
